...
class AuthNotifier extends AsyncNotifier<UserEntity?> {
  ...
  Future<void> sendOTP(String phoneNumber) async {
    ...
    if (kDebugMode) {
      debugPrint('📲 AuthProvider.sendOTP called');
    }
    ...
    try {
      await ref.read(authRepositoryProvider).sendOTP(
        ...
        onCodeSent: (String verificationId) {
          if (kDebugMode) {
            debugPrint('📲 onCodeSent callback received in provider: $verificationId');
          }
          _verificationId = verificationId;
          _isOtpSent = true;
          state = const AsyncData(null);
          if (kDebugMode) {
            debugPrint('📲 State set to data(null), isOtpSent: $_isOtpSent');
          }
        },
      );
      if (kDebugMode) {
        debugPrint('📲 sendOTP initiation finished');
      }
